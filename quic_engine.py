...
class EchoServerRequestHandler:
    ...
    async def launch_echo(self):
        qc = EchoQuicConnection(
            self.send,
            self.receive,
            self.close,
            self.connection.get_next_available_stream_id,
        )
        await echo_server.chat_server_proto(self.scope, qc)
...
class EchoClientRequestHandler(EchoServerRequestHandler):
    ...
    async def launch_echo(self):
        ...
        await echo_client.chat_client_proto(self.scope, qc)
